...
retail_df %>% 
    select(subdiv_name, parent_shop, trade_area, revenue, revenue_change, 
           discount_share, rev_per_sq, gross_profit, profitability, gmros) %>% 
    filter(subdiv_name != "ВСЕГО") %>%
...
retail_df %>%
    select(subdiv_name, parent_shop, revenue, total_checks, total_checks_change,
           one_item_checks_share, avr_check_sum, avr_check_change,
           avr_check_sku, avr_unit_sum) %>%
    filter(subdiv_name != "ВСЕГО") %>%
...
sale_category %>% 
    ...
    geom_text(aes(y = cat_profit), size = 3) +
    coord_flip() +
...
### Кластеризация магазинов по категор.направлениям в чеках {data-width=500}
...
### Конверсия в супермаркетах {data-width=500}