...
delivery_data_clr <- df_delivery %>% 
    ...
    left_join(ref_subdiv_category, by = "subdiv_name") %>% 
    filter(!(to_intermediate == TRUE &
                 subdiv_category %in% " Супермаркети  В2В" &
                 store_id %in% distribution_stores)) %>% 
    mutate(flag_intermediate = ifelse(to_intermediate == TRUE &
                                          !subdiv_category %in% " Супермаркети  В2В",
                                      1,
                                      0),
           lead_time = as.integer(date_delivery - date_cust_order_doc) +
               flag_intermediate)

total_lead_time <- round(mean(delivery_data_clr$lead_time), 1)

subdiv_lead_time <- delivery_data_clr %>% 
    group_by(subdiv_name) %>% 
...
sale_df <- sale_df_raw %>% 
    ...
    mutate(avr_cust_items = ifelse(subdiv_name == "ВСЕГО",
           ...
           customers_growth = ifelse(cust_prev_year == 0,
                                     0,
                                     round(subdiv_customers / cust_prev_year, 2)
                                     ),
...
debt_subdiv <- debt_finish_df %>% 
    ...
    pivot_longer(cols = 2:5,
                 names_to = "term",
                 values_to = "term_sum") %>% 
    mutate(term_sum = ifelse(term_sum < 0,
                             0,
                             term_sum)) %>% 
    group_by(subdiv_name) %>% 
...
nb.cols <- length(top_segment)
mycolors <- colorRampPalette(RColorBrewer::brewer.pal(7, "Blues"))(nb.cols)
...
avr_cust_sum_month %>% 
    ...
    scale_y_continuous(
        ...
        sec.axis = sec_axis(~./coeff,
                            name = "Середня ціна продажу паперу оф.(грн)")) + 
...
sale_df %>%
    select(subdiv_name, subdiv_category, subdiv_sum, subdiv_customers,
           customers_growth, avr_cust_sum, avr_cust_items, avr_cust_groups,
           avr_unit_sum) %>%
    filter(!subdiv_name %in% c("ВСЕГО", "Інші"),
           subdiv_sum > 0) %>%
    reactable(striped = TRUE,
        ...
        columns = list(
            ...
            subdiv_customers = colDef(aggregate = "sum",
                                      ...
                                      format = colFormat(separators = TRUE,
                                                     digits = 0)),
            customers_growth = colDef(name = "TP",
                                      maxWidth = 50, 
                                      style = function(value) {
                                         if (value > 1) {
                                             color <- "#008000"
