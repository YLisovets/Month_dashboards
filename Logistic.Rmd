...
store_space_workload_fn <- function(store) {
    ...
    not_delivered_sale_doc_items <- not_delivered_doc %>% 
        ...
        left_join(select(sale_raw, sale_doc_nmbr, date, item_id, item_qty),
              by = c("nmbr_sale_doc" = "sale_doc_nmbr",
                     "date_sale_doc" = "date")) %>% 
        select(item_id, item_qty_sale = item_qty)
    ...
    select(item_id, item_qty_moving = item_qty)       
...
fig1 <- total_exp_share %>% 
    ggplot(aes(x = month, y = exp_share, group = 1)) + 
...
fig2 <- total_exp_share %>% 
    ggplot(aes(x = month, y = exp_per_units_sold, group = 1)) + 
    geom_line(size = 1.5, color = "steelblue") +
    geom_point(shape = 21, size = 3, fill = "steelblue") +
    theme_minimal() +
    labs(y = "",
...
sale_cur_month <- sale_raw %>% 
    ...
    mutate(store = ifelse(!is.na(check_store),
                          check_store,
                          sale_doc_store),
...
sale_by_organization <- sale_raw %>% 
    ...
    mutate(type_organization = case_when(
        organiz_id == "000000001" ~ "КОРВЕТ ф1",
        TRUE                      ~ "ПП"
    )) %>% 
           ...
           !(!is.na(nmbr_moving_out) &
                 ...
                 (deliv_order_store_id %in% c("000000001") |
                      (!is.na(intermediate_store) & intermediate_store ==
                           "Головний склад"))),
           !(route_id == "000000106" & !is.na(nmbr_moving_out))
           )
...
deliveries_summary <- cleared_deliveries %>% 
    ...
    summarise(driver = first(driver),
              ...
              customers_qty = n_distinct(customer_id, deliv_address, na.rm = TRUE),
...
store_geocode <- unique(distance_moving_out_data$store_address) %>% 
    str_replace("м.", "") %>% 
    geo(method = "osm")

distance_fn <- function(data) {
    route <- data %>% 
        left_join(store_geocode, by = c("store_address" = "address")) %>% 
        st_as_sf(coords = c("long", "lat"), crs = 4326) %>% 
        osrmTrip(returnclass = "sf")
...
fig_suppl_inspection_by_store <- suppl_inspection_by_store %>% 
    ...
    geom_bar(stat = "identity", fill = "lightblue") +
    geom_text(
        aes(label = paste0(avr_inspection_duration, "(", inspection_qty, ")")),
        size = 7,
        hjust = 1, position = position_dodge(.9)
    ) +
    coord_flip() +
...
fig_suppl_inspection_by_supplier <- suppl_inspection_by_supplier %>% 
    ...
    geom_text(
        ...
        size = 6,
        hjust = 0.9, position = position_dodge(.9)
    ) +
    coord_flip() +
    theme_minimal() +
    labs(x = "", y = "") +
...
moving_inspection <- df_inspection %>% 
    ...
    summarise(store = first(store_recipient),
              ...
              inspector = first(user_id),
              inspection_duration = ifelse(
                  as.Date(time_start) >= as.Date("4022-01-01"),
                  last(time_finish) - first(time_start),
                  0),
              lines = n(),
              total_units = sum(item_qty),
              deviation_lines = sum(deviation_qty != 0))
...
avr_moving_inspection_time = round(median(as.numeric(
    moving_inspection$inspection_duration[moving_inspection$inspection_duration > 0]))/60, 1)
...
fig_customer_assembly_by_store <- customer_assembly_by_store %>% 
    ...
    geom_bar(stat = "identity", fill = "lightblue") +
    geom_text(
        aes(label = paste0(avr_assembly_duration, "(", assembly_qty, ")")),
        size = 7,
        hjust = 1, position = position_dodge(.9)
    ) +
    coord_flip() +
...
stores_worker_data <- bind_rows(supplier_inspection_user_data,
    ...
    mutate(first_doc_time = hour(min(time_start)),
           last_doc_time  = hour(max(time_finish[date_start == date_finish]))) %>% 
    ungroup()
...
store_operations <- stores_worker_df %>% 
    ...
    summarise(total_operation_time = sum(user_oper_working_time)) %>% 
    ungroup() %>% 
...
fig_store_operations <- store_operations %>% 
    ...
    scale_y_continuous(breaks = seq(0, 1, .2), 
                       label = scales::percent) +
    geom_text(aes(label = scales::percent(operation_share)),
              size = 3.5,
              position = position_stack(vjust = 0.5)) +
    scale_fill_brewer(palette = "Pastel1") +
    coord_flip() +
...
store_space__workload <- tibble(store = distribution_stores) %>% 
    mutate(data = map(store, store_space_workload_fn)) %>% 
    unnest(cols = c(data)) %>% 
    left_join(select(ref_store, store_id, store_name),
              by = c("store" = "store_id"))

fig_store_space_workload <- store_space__workload %>% 
...
### Найгірші підрозділи з рівеня доступності запасів
...
### Коеф-нт використання вантажопідйомності (зважений)
...
### Склади-віправ. з найгіршим середнім часом на приймання, хв.(кіл.док.) {.no-padding}
...
### Склади-віправ. з найгіршим % відхілень у поставках (заг.кіл.рядків) {.no-padding}
