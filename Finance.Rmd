...
expenses_fn <- function(date_start, date_finish) {
    ...
    delivery_vehicle <- delivery_df %>% 
        ...
        left_join(select(ref_subdiv, subdiv_id,
                         vehicle_subdiv_name = subdiv_name))
    ...
    expenses_df <- expenses_raw %>% 
        group_by(subdiv_name, subdiv_id, cost_item, expItems_name) %>% 
        summarise(expense_sum = sum(exp_sum, na.rm = TRUE)) %>% 
        ungroup() %>% 
        left_join(select(ref_subdiv, subdiv_id, subdiv_parent),
                  by = "subdiv_id") %>% 
        ...
        mutate(expenses_group = case_when(
            ...
            cost_item %in% c("08 Автовитрати, запчастини",
                             ...
                             "24 Страхування") &
                parent_2 %in% c("Регіональні ма`газини",
...
sale_fn <- function(date_start, date_finish) {
    ...
    cost_df <- cost_data(date_start,
        ...
        mutate(project = case_when(
            parent_project_id %in% "000000009" |
                (is.na(project_id) & !is.na(nmbr_refund_doc)) ~ "Сервис",
            !is.na(nmbr_check_doc)                            ~ "Розница",
            project_id %in% c("000000003",
                              "000000002")                    ~ "Розница",
            TRUE                                              ~ "Корректировка"),
               subdiv_name = ifelse(project != "Корректировка",
                                    subdiv_name,
    ...
    sale_df <- sale_data_total(date_start,
        ...
        mutate(project = ifelse(
            parent_project_id %in% "000000009",
            "Сервис",
            ifelse(parent_itemId %in% "000000185",
                   ifelse(
                       str_detect(item_name,
                                  "(Відшкодування)|(Орендна плата)") &
                           !item_name %in% c(
                               "Відшкодування по інвентаризації",
                               "Відшкодування  вартості доставки товару"),
                       "Оренда",
                       item_name),
                    ifelse(project_id %in% "000000003" |                # Сотрудники 
                               parent_project_id %in% "000000010",
                           "Розница",
                           project))),
...
sales_month_fn <- function(data) {
    df <- data %>% 
        ...
        mutate(sale_sum = ifelse(group == "Інші операц.доходи" &
                                     !is.na(cost_sum),
                                 -cost_sum,
                                 sale_sum),
               cost_sum = ifelse(group == "Інші операц.доходи" &
                                     !is.na(cost_sum),
                                 0,
                                 cost_sum)) %>% 
        group_by(group) %>%
...
debt_customers_fn <- function(data) {
    total_sum <- data %>% 
        ...
        left_join(select(ref_projects, project_id, parent_project_id)) %>% 
        filter(type_order == 1,
               !customer_folder_id %in% c("000423140",    # РЕГИОНАЛЬНЫЕ МАГАЗИНЫ
                                          "001004385",    # КІНЦЕВИЙ ПОКУПЕЦЬ
                                          "000300700"),   # СПЕЦИАЛИСТЫ
...
subdiv_vat_calculation <- function(data, total_vat) {
    ...
    mutate(across(where(is.numeric), ~ifelse(is.na(.), 0, .))) %>% 
    group_by(subdiv_name) %>% 
    summarise(sale_sum = sum(sale_sum),
              cost_sum = sum(cost_sum)) %>% 
    ungroup() %>%     
    mutate(gross_profit = sale_sum - cost_sum,
    ...
    mutate(cf_item_id = ifelse(
        ...
        ifelse(cf_item_id == "00013" & cash_id %in% c("000000004",
               ...
               ifelse(cf_item_id == "00014" & type_money_order == 0 &
                          oco_responsible %in% "Куц Вікторія",
                      "00099",
                      ifelse(cf_item_id == "00013" & type_money_order == 0 &
                                 ico_responsible %in% "Куц Вікторія",
                             "00142",
                             ifelse(cf_item_id == "00089" & # Інші доходи(Оренда)
                                        partner_parent %in% "000423140",
                                    "00147",
                                    ifelse(cf_item_id == "00104" & # Оренда
                                               partner_parent %in% "000423140",
                                           "00147",
                                           ifelse(cf_item_id == "00001" & # Оплата постач.
                                                      partner_parent %in% "000423140",
                                                 "00147",
                                                 ifelse(cf_item_id == "00002" & # Оплата покуп.
                                                            partner_parent %in% "000423140",
                                                            "00147",
                                                            cf_item_id))))))))
    ) %>% 
...
investment_activities_income <- c("00132", "00133")
...
money_in_way_start_month <- get_money_on_way(date_start) %>% 
    mutate(type_cash = case_when(
               organization_id == "000000001" ~ "Корвет безгот.",
               .default = "Інші"
           )) %>% 
    select(type_cash, acc_bal_grn = balance_sum)

cash_balance_start_month_data <- cash_balance_data(date_start + years(2000)) %>% 
    mutate(type_cash = case_when(
        organiz_id == "000000001" & type_acc == "безготівковий" ~ "Корвет безгот.",
        .default = "Інші"
    )) %>% 
    select(type_cash, acc_bal_grn) %>% 
    bind_rows(money_in_way_start_month) %>% 
    group_by(type_cash) %>% 
    summarise(type_sum = sum(acc_bal_grn))

cash_balance_start_month <- cash_balance_start_month_data %>%     
    summarise(total_balance = sum(type_sum)) %>% 
    pull()


money_in_way_finish_month <- get_money_on_way(date_finish) %>% 
    mutate(type_cash = case_when(
               organization_id == "000000001" ~ "Корвет безгот.",
               .default = "Інші"
           )) %>% 
...
cash_balance_finish_data <- cash_balance_data(date_finish + years(2000)) %>% 
    mutate(type_cash = case_when(
        organiz_id == "000000001" & type_acc == "безготівковий" ~ "Корвет безгот.",
        .default = "Інші"
    )) %>% 
...
cash_balance_finish <- cash_balance_finish_data %>% 
    summarise(total_balance = sum(type_sum)) %>% 
    pull()
...
cash_flow_tbl <- cash_flow_df %>%
    ...
    add_row(item = "Спонсорська допомога",
            sum_moving = ifelse(is_empty(sponsored_cur_month),
                                ...
                                sponsored_cur_month),
            .after = 7) %>% 
    filter(!(item %in% c("Поповнення обігових коштів",
                         "Виплата дівідендів",
...
cash_flow_tbl <- cash_flow_tbl %>% 
    add_row(item = "Грошові кошти на початок",
            ...
            .before = 1) %>% 
    add_row(item = "   у т.ч. на рах.Корвет",
            sum_moving = round(cash_balance_start_month_data$type_sum[2] / 1000, 1),
            .before = 2) %>%
    add_row(item = "Грошові кошти на кінець",
            sum_moving = round(cash_balance_finish / 1000, 1)) %>% 
    add_row(item = "   у т.ч. на рах.Корвет",
            sum_moving = round(cash_balance_finish_data$type_sum[2] / 1000, 1))
...
money_in_way_start_year <- get_money_on_way(date_start_year) %>% 
    mutate(type_cash = case_when(
               organization_id == "000000001" ~ "Корвет безгот.",
               .default = "Інші"
           )) %>% 
...
cash_balance_start_year_data <- cash_balance_data(date_start_year + years(2000)) %>% 
    mutate(type_cash = case_when(
        organiz_id == "000000001" & type_acc == "безготівковий" ~ "Корвет безгот.",
        .default = "Інші"
    )) %>% 
...
cash_balance_start_year <- cash_balance_start_year_data %>%    
    summarise(total_balance = sum(type_sum)) %>% 
    pull()
...
purchases_current_month <- purchases_data(date_start + years(2000),
                                      date_finish + years(2000)) %>% 
    filter(!str_detect(contract_name, "6851")) %>% 
...
profitability %>% 
    ...
    geom_text(aes(y = `Чистий прибуток`), vjust = -0.5, size = 3.5) +
...
cash_flow_tbl %>% 
    reactable(
        ...
        rowStyle = function(index) {
            if (index %in% c(3, 6, 7, cash_flow_nrow+2)) 
