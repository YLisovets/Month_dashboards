...
names(plan_indicators)[c(2,3)] <- c("checks_plan", "check_sku_plan")
...
plan_retail_data <- sale_plan_retail(date_start + years(2000),
    ...
    mutate(subdiv_name = ifelse(subdiv_name == "Копіцентр ХМ СМ",
                                ...
                                subdiv_name)) %>%
    group_by(subdiv_name) %>% 
    summarise(plan_sum = sum(plan_sale),
...
work_duration <- sale_check_df %>% 
    ...
    group_by(subdiv_name) %>% 
    summarise(period_work_time = sum(work_duration, na.rm = TRUE))
...
retail_df <- sale_check_df %>% 
    ...
    left_join(empl_work_time_retail, by = "subdiv_name") %>%
    left_join(plan_retail_data) %>% 
    ...
    mutate(plan_implement = ifelse(plan_sum == 0,
                                   0,
                                   round(revenue / plan_sum, 3)),
           revenue_growth = ifelse(revenue_prev_year == 0,
                                   0,
                                   ifelse(revenue_prev_year > 0, 1, 0) *
                                       round(revenue / revenue_prev_year, 2)),
           checks_nmbr_impl = ifelse(plan_ind_nrow > 0, 1, 0) *
                                     round(total_checks / checks_plan,
                                           3),
           total_checks_growth = ifelse(checks_prev_year == 0,
                                        0,
                                        ifelse(checks_prev_year > 0, 1, 0) *
                                            round(total_checks / 
                                                      checks_prev_year, 2)),
           one_item_checks_share = round(one_item_checks_qty/total_checks, 3),
           avr_check_sum = round(revenue / total_checks, 1),
           avr_check_impl = ifelse(plan_ind_nrow > 0, 1, 0) *
                                   round(avr_check_sum /
                                      (plan_sum / checks_plan), 3),
           avr_check_growth = ifelse(checks_prev_year == 0,
                                     0,
                                     ifelse(checks_prev_year > 0, 1, 0) *
           ...
           row_n = row_number(),
           check_sku_plan = ifelse(row_n == max(row_n),
                                   weighted.mean(check_sku_plan[1:(max(row_n)-1)],
                                                 checks_plan[1:(max(row_n)-1)]),
                                   check_sku_plan),
           avr_check_sku_impl = ifelse(plan_ind_nrow > 0, 1, 0) *
                                       round(avr_check_sku /
                                           (check_sku_plan), 3),
...
avr_check_sku_plan <- retail_df$check_sku_plan[total_shops + 1]
avr_check_sku_inplemention <- retail_df$avr_check_sku_impl[total_shops + 1] * 100
avr_check_plan <- ifelse(plan_ind_nrow > 0,
                         round(retail_df$plan_sum[total_shops + 1] /
                            retail_df$checks_plan[total_shops + 1], 1),
                         0)
...
sale_history_by_year %>% 
    ...
    plot_time_series(date, sales,
                     ...
                     .title = "",
                     .interactive = TRUE) %>% 
    layout(xaxis = list(tickformat="%m\n%Y"))
```
...
retail_df %>% 
    select(subdiv_name, parent_shop, trade_area, revenue, plan_implement,
           revenue_growth, net_discount_share, rev_per_sq, gross_profit,
           profitability, gmros) %>% 
    filter(subdiv_name != "ВСЕГО") %>%
    ...
    reactable(striped = TRUE,
        ...
        columns = list(
            ...
            revenue = colDef(aggregate = "sum",
                             ...
                             format = colFormat(separators = TRUE,
                                                digits = 0)),
            plan_implement = colDef(name = "Викон.плану",
                                    maxWidth = 70, 
...
### Сума середнього чеку за останні 4 тижня {data-width=400}
...
last_4week_data %>% 
    ...
    scale_x_date(date_labels = "%b %d",
                 minor_breaks = NULL,
                 breaks = seq(min(last_4week_data$week_last_day),
                             max(last_4week_data$week_last_day),
                             length=4)) +
...
```{r}
retail_df %>%
    select(subdiv_name, parent_shop, revenue, total_checks, total_checks_growth,
           one_item_checks_share, share_regular_cust, avr_check_sum, 
           avr_check_growth, avr_check_sku, avr_unit_sum,
